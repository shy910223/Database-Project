...
public:
    ...
    void retrieveRecord(void *data, vector<Attribute> &attrs, string attr, void *dataout);
...
public:
    ...
    void mySetIterator(const string& attrName, CompOp compOp, const void* value) {
        iter->close();
        delete iter;
        iter = new RM_ScanIterator();
        rm.scan(tableName, attrName, compOp, value, attrNames, *iter);
    };
...
private:
    ...
    vector<int> aggIntValues;
    vector<float> aggRealValues;
    Attribute aggAttr;
    AggregateOp op;
public:
